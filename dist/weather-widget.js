...
class Y extends HTMLElement {
  ...
  #l({ cityName: t, iconName: a, tempText: s }) {
    this.#t && (this.#t.loading.hidden = !0, this.#t.error.hidden = !0, this.#t.content.hidden = !1, this.#t.city.textContent = t, this.#t.icon.src = D[a] || d, this.#t.temp.textContent = s);
  }
  async #o() {
    const t = this.city;
    if (!t) {
    ...
    try {
      ...
      const s = new URL("https://api.openweathermap.org/geo/1.0/direct");
      s.searchParams.set("q", t), s.searchParams.set("limit", "1"), s.searchParams.set("appid", a);
      const l = await fetch(s, { signal: this.#n.signal });
      if (!l.ok) throw new Error("City not found");
      const c = await l.json(), i = c && c[0];
      if (!i) throw new Error("City not found");
      const o = new URL(
        "https://api.openweathermap.org/data/2.5/weather"
      );
      o.searchParams.set("lat", String(i.lat)), o.searchParams.set("lon", String(i.lon)), o.searchParams.set("units", "metric"), o.searchParams.set("appid", a);
      const r = await fetch(o, {
        signal: this.#n.signal
      });
      if (!r.ok) throw new Error("Weather not available");
      const e = await r.json(), h = e?.main?.temp, w = this.scale === "F" ? Math.round(h * 9 / 5 + 32) : Math.round(h), g = e?.weather?.[0]?.icon, v = g && J[g] || "help-circle", m = K(v, this.iconStyle), p = e?.name || t;
      this.#l({
        cityName: p,
        iconName: m,
      ...
      const s = a instanceof Error ? a.message : "Failed to fetch weather";
      this.#s(s);
    }
  }
}
